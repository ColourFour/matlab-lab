(function () {
  ...
  function route() {
    ...
    if(match && courses[match[1]]){
      ...
      if(unlocked(course)){
        ...
        if(index>=0 && (review || index<=state.completed.length))return {page:'lesson',index};
      }
      notice=unlocked(course)?tr('Complete the earlier missions to unlock this lesson.','请先完成前面的任务，解锁本课。'):tr('Finish Project '+courses[course.requires].number+' to unlock this project. Preview the missions below.','完成项目 '+courses[course.requires].number+' 后解锁此项目。可先预览下方任务。');
  ...
  function lessonStatus(i){if(review)return i<state.completed.length?'done':'next';return !unlocked(course)?'locked':i<state.completed.length?'done':i===state.completed.length?'next':'locked';}
