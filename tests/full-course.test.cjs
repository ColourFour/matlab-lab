...
test('Seven bilingual projects have 48 guided lessons, eight open sections and real downloads',()=>{
 ...
 for(const c of Object.values(courses)){
  ...
  for(const l of c.lessons){
   ...
   if(c.kind==='capstone'){assert.ok(l.open&&l.prompts.length===2&&l.checks.length===2);for(const k of ['title','short','goal','see','understand','compare'])assert.ok(l[k].en&&l[k].zh);continue;}
